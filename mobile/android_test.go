...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.", new NodeConfig());
			node.start();
			...
			WabeiClient ec = node.getWabeiClient();
	...
	// signatures too. Regression test for go- issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org//gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.", "github.com/wabei/go-wabei/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
