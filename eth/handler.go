...
import (
	...
	"github.com/wabei/go-wabei/eth/fetcher"
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Wabei peer", "peer", id)

	// Unregister the peer from the downloader and Wabei peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Wabei protocol")
	...
	log.Info("Wabei protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Wabei peer connected", "name", p.Name())

	// Execute the Wabei handshake
	var (
	...
	if err := p.Handshake(pm.networkId, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("Wabei handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Wabei peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Wabei message handling failed", "err", err)
			return err
		}
	}
}
