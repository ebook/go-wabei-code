...
// NewProtocolManager returns a new  sub protocol manager. The Wabei sub protocol manages peers capable
// with the  network.
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Wabei protocol")
	...
	log.Info("Light Wabei protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light Wabei peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light Wabei handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light Wabei peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Wabei message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Wabei message arrived", "code", msg.Code, "bytes", msg.Size)
...
// NodeInfo represents a short summary of the Wabei sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Wabei network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
