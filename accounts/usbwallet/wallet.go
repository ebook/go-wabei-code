...
type wallet struct {
	...
	deriveChain    .ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain .ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
